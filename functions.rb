...
def query_user_for_date_range
  ...
  # Until we have a start_date and end_date execute this loop.
...
# We know that any single date (start_date or end_date) must be after the "DATA_START_DATE" and before today.
def date_valid?(date)
  valid_dates = Date.parse(DATA_START_DATE)..Date.today
  # Does the "valid_dates" "cover" the passed in date?
